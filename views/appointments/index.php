                ...
                <div class="card-body border-bottom pt-3 pb-3">
                    <form method="GET" action="index.php" class="row align-items-end">
                        ...
                        <div class="col-md-1 mb-2 d-flex align-items-end">
                            <button type="submit" class="btn btn-sm btn-secondary btn-block mr-1" title="Filter">
                                <i class="fas fa-filter"></i>
                            </button>
                            <a href="index.php?page=appointments" class="btn btn-sm btn-outline-secondary btn-block" title="Clear">
                            ...
                            <?php else: ?>
                                <?php foreach ($appointments as $appt): ?>
                                    <tr>
                                        ...
                                        <td>Dr. <?= sanitize($appt['doctor_name']) ?></td>
                    ...
                    <div class="card-footer clearfix">
                        <ul class="pagination pagination-sm m-0 float-right">
                            <?php if ($paginator->hasPrev()): ?>
                                <li class="page-item">
                                    <a class="page-link" href="?page=appointments&p=<?= $paginator->getPrevPage() ?>&status=<?= $currentStatus ?>&doctor_id=<?= $currentDoctor ?>&patient_name=<?= urlencode($patientName) ?>&start_date=<?= $startDate ?>&end_date=<?= $endDate ?>">
                                        &laquo;
                                    </a>
                                </li>
                            <?php endif; ?>
                            <?php for ($i = 1; $i <= $paginator->totalPages(); $i++): ?>
                                <li class="page-item <?= $i == $paginator->currentPage() ? 'active' : '' ?>">
                                    <a class="page-link" href="?page=appointments&p=<?= $i ?>&status=<?= $currentStatus ?>&doctor_id=<?= $currentDoctor ?>&patient_name=<?= urlencode($patientName) ?>&start_date=<?= $startDate ?>&end_date=<?= $endDate ?>">
                                        <?= $i ?>
                                    </a>
                                </li>
                            <?php endfor; ?>
                            <?php if ($paginator->hasNext()): ?>
                                <li class="page-item">
                                    <a class="page-link" href="?page=appointments&p=<?= $paginator->getNextPage() ?>&status=<?= $currentStatus ?>&doctor_id=<?= $currentDoctor ?>&patient_name=<?= urlencode($patientName) ?>&start_date=<?= $startDate ?>&end_date=<?= $endDate ?>">
                                        &raquo;
                                    </a>
                                </li>
                            <?php endif; ?>
